...
const { createUserContainer, cleanupInactiveContainers, getUserContainers, removeAllContainer, removeContainerById, runDockerApplicationWithLogs } = require('./docker');
...
app.get('/remove-container', async (req, res) => {
  try {
      let removed = [];
...
app.post("/api/remove-container",async(req,res)=>{
  try {
...
server.listen(4000, () => console.log('API + WS running on http://localhost:4000'));
